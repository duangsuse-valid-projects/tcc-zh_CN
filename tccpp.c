...
static char tok_two_chars[] =
    "<=\236>=\235!=\225&&\240||\241++\244--\242==\224<<\1>>\2+=\253-=\255*="
    "\252/=\257%=\245&=\246^=\336|=\374->\313..\250##\266";
...
/* return the current character, handling end of block if necessary
   (but not stray) */
static int handle_eob(void) { return tcc_peekc_slow(file); }
...
static void tok_str_free(int *str) { tcc_free(str); }
...
/* defines handling */
static inline void define_push(int v, int macro_type, int *str,
                               Sym *first_arg) {
  Sym *s;
...
static void parse_define(void) {
  ...
  v = tok;
  if (v < TOK_IDENT) error("invalid macro name '%s'", get_tok_str(tok, &tokc));
  ...
  if (tok == '(') {
    ...
    while (tok != ')') {
      ...
      if (varg < TOK_IDENT) error("badly punctuated parameter list");
  ...
  if (spc == 1) --str.len; /* remove trailing space */
  tok_str_add(&str, 0);
#ifdef PP_DEBUG
...
static CachedInclude *search_cached_include(TCCState *s1, int type,
                                            const char *filename) {
  CachedInclude *e;
  int i, h;
  ...
  for (;;) {
    ...
    if (e->type == type && !PATHCMP(e->filename, filename)) return e;
    i = e->hash_next;
  }
  return NULL;
}
...
static void preprocess(int is_bof) {
  ...
  parse_flags =
      PARSE_FLAG_PREPROCESS | PARSE_FLAG_TOK_NUM | PARSE_FLAG_LINEFEED;
  next_nomacro();
redo:
  switch (tok) {
    case TOK_DEFINE:
    ...
    case TOK_LINE:
      ...
      file->line_num =
          tokc.i - 1; /* the line number will be incremented after */
      next();
      if (tok != TOK_LINEFEED) {
...
redo_no_start:
  ...
  switch (c) {
    ...
    parse_ident_fast:
      ...
      for (;;) {
        c = *p;
        if (!isidnum_table[c - CH_EOF]) break;
        h = TOK_HASH_FUNC(h, c);
        p++;
      }
      if (c != '\\') {
  ...
  while (1) {
    ...
    if (t == '#') {
      ...
      if (s) {
        ...
        while (*st) {
          TOK_GET(t, st, cval);
          if (!check_space(t, &spc)) cstr_cat(&cstr, get_tok_str(t, &cval));
        }
        cstr.size -= spc;
        cstr_ccat(&cstr, '\0');
#ifdef PP_DEBUG
      ...
      if (s) {
        ...
        if (*macro_str == TOK_TWOSHARPS || last_tok == TOK_TWOSHARPS) {
          ...
          /* XXX: test of the ',' is not 100%
             ...
             problems */
          if (gnu_ext && s->type.t && last_tok == TOK_TWOSHARPS &&
...
static int macro_subst_tok(TokenString *tok_str, Sym **nested_list, Sym *s,
  ...
  add_cstr1:
    ...
    redo:
      if (macro_ptr) {
        p = macro_ptr;
        while (is_space(t = *p) || TOK_LINEFEED == t) ++p;
          ...
          if (tok == TOK_PPNUM) {
            ...
            if (t == TOK_PPNUM) {
              ...
              for (;;) {
                ...
                if (c == '\0') break;
                p++;
                if (!isnum(c) && !isid(c)) goto error_pasting;
              }
            }
...
static void macro_subst(TokenString *tok_str, Sym **nested_list,
  ...
  while (1) {
    ...
    if (s != NULL) {
      ...
      ml.p = macro_ptr;
      if (can_read_stream) ml.prev = *can_read_stream, *can_read_stream = &ml;
      macro_ptr = (int *)ptr;
      tok = t;
