...
static unsigned long stuff_const(unsigned long op, unsigned long c) {
  ...
  switch (op & 0x1F00000) {
    ...
    case 0x1E00000:  // mvn
      ...
      break;
    case 0x200000:  // xor
      if (c == ~0) return (op & 0xF010F000) | ((op >> 16) & 0xF) | 0x1E00000;
      break;
    case 0x0:  // and
      if (c == ~0) return (op & 0xF010F000) | ((op >> 16) & 0xF) | 0x1A00000;
    case 0x1C00000:  // bic
      try_neg = 1;
      negop = op ^ 0x1C00000;
      nc = ~c;
      break;
    case 0x1800000:  // orr
      if (c == ~0) return (op & 0xFFF0FFFF) | 0x1E00000;
      break;
  }
  do {
    unsigned long m;
    ...
    for (i = 2; i < 32; i += 2) {
      m = (0xff >> i) | (0xff << (32 - i));
      if (!(c & ~m)) return op | (i << 7) | (c << i) | (c >> (32 - i));
    }
    op = negop;
    c = nc;
  } while (try_neg--);
...
void stuff_const_harder(unsigned long op, unsigned long v) {
  ...
  if (x)
    ...
    ;
    for (i = 1; i < 16; i++) a[i] = (a[i - 1] >> 2) | (a[i - 1] << 30);
    for (i = 0; i < 12; i++)
...
static void calcaddr(unsigned long *base, int *off, int *sgn, int maxoff,
                     unsigned shift) {
  ...
  if (fr & VT_LVAL) {
    ...
    if (v == VT_LOCAL) {
      if (is_float(ft)) {
        ...
        op = 0xED100A00; /* flds */
        if (!sign) op |= 0x800000;
        if ((ft & VT_BTYPE) != VT_FLOAT) op |= 0x100; /* flds -> fldd */
        o(op | (vfpr(r) << 12) | (fc >> 2) | (base << 16));
#else
        op = 0xED100100;
        if (!sign) op |= 0x800000;
#if LDOUBLE_SIZE == 8
        if ((ft & VT_BTYPE) != VT_FLOAT) op |= 0x8000;
#else
        ...
        op = 0xE1500090;
        if ((ft & VT_BTYPE) == VT_SHORT) op |= 0x20;
        if ((ft & VT_UNSIGNED) == 0) op |= 0x40;
        if (!sign) op |= 0x800000;
        ...
        op = 0xE5100000;
        if (!sign) op |= 0x800000;
        if ((ft & VT_BTYPE) == VT_BYTE) op |= 0x400000;
    ...
    if (v == VT_CONST) {
      ...
      if (fr & VT_SYM || !op) {
        ...
        o(0xEA000000);
        if (fr & VT_SYM) greloc(cur_text_section, sv->sym, ind, R_ARM_ABS32);
        o(sv->c.ul);
      } else
        o(op);
      return;
  ...
  if (fr & VT_LVAL || fr == VT_LOCAL) {
    ...
    if (v == VT_LOCAL) {
      if (is_float(ft)) {
        ...
        op = 0xED000A00; /* fsts */
        if (!sign) op |= 0x800000;
        if ((ft & VT_BTYPE) != VT_FLOAT) op |= 0x100; /* fsts -> fstd */
        o(op | (vfpr(r) << 12) | (fc >> 2) | (base << 16));
#else
        op = 0xED000100;
        if (!sign) op |= 0x800000;
        ...
        if ((ft & VT_BTYPE) != VT_FLOAT) op |= 0x8000;
#else
        if ((ft & VT_BTYPE) == VT_DOUBLE) op |= 0x8000;
        if ((ft & VT_BTYPE) == VT_LDOUBLE) op |= 0x400000;
#endif
        o(op | (fpr(r) << 12) | (fc >> 2) | (base << 16));
#endif
        return;
...
static void gcall_or_jmp(int is_jmp) {
  ...
  if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
    ...
    if (x) {
      ...
      if (!is_jmp) o(0xE28FE004);  // add lr,pc,#4
    ...
    r = gv(RC_INT);
    if (!is_jmp) o(0xE1A0E00F);  // mov lr,pc
  ...
  for (i = 0; i < nb_args; i++) {
    ...
    if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) {
      ...
      if (vtop->type.t == VT_VOID) {
        if (s == RC_INT) o(0xE24DD004); /* sub sp,sp,#4 */
        vtop--;
      } else
#endif
          if (s == RC_INT) {
...
void gfunc_prolog(CType *func_type) {
  ...
  n = 0;
  addr = 0;
  if ((func_vt.t & VT_BTYPE) == VT_STRUCT && type_size(&func_vt, &align) > 4) {
    func_vc = addr;
    addr += 4;
    n++;
  }
  ...
  if (func_type->ref->c == FUNC_ELLIPSIS) n = 4;
  if (n) {
    if (n > 4) n = 4;
#ifdef TCC_ARM_EABI
    n = (n + 1) & -2;
  ...
  if (diff > 12) {
    ...
    if (x)
      *(unsigned long *)(cur_text_section->data + func_sub_sp_offset) = x;
      ...
      o(diff);
      *(unsigned long *)(cur_text_section->data + func_sub_sp_offset) =
...
static int is_zero(int i) {
  if ((vtop[i].r & (VT_VALMASK | VT_LVAL | VT_SYM)) != VT_CONST) return 0;
...
void gen_opf(int op) {
  ...
  switch (op) {
    ...
    default:
      ...
      x |= 0xB40040;                               /* fcmpX */
      if (op != TOK_EQ && op != TOK_NE) x |= 0x80; /* fcmpX -> fcmpeX */
      if (is_zero(0)) {
        vtop--;
...
static int is_fconst() {
  long double f;
  int r;
  if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) != VT_CONST) return 0;
...
void gen_opf(int op) {
  ...
  x = 0xEE000100;
#if LDOUBLE_SIZE == 8
  if ((vtop->type.t & VT_BTYPE) != VT_FLOAT) x |= 0x80;
#else
...
void gen_cvt_itof1(int t) {
  ...
  if (bt == VT_INT || bt == VT_SHORT || bt == VT_BYTE) {
    ...
    r2 <<= 12;
    if (!(vtop->type.t & VT_UNSIGNED)) r2 |= 0x80; /* fuitoX -> fsituX */
    o(0xEEB80A40 | r2 | T2CPR(t));                 /* fYitoX*/
#else
    r2 = fpr(vtop->r = get_reg(RC_FLOAT));
    if ((t & VT_BTYPE) != VT_FLOAT) dsize = 0x80;   /* flts -> fltd */
...
void gen_cvt_ftoi(int t) {
  ...
  if (t == VT_INT) {
...
#else
    if (u) {
      if (r2 == VT_FLOAT) func = TOK___fixunssfsi;
#if LDOUBLE_SIZE != 8
    ...
    if (r2 == VT_FLOAT) func = TOK___fixsfdi;
#if LDOUBLE_SIZE != 8
    else if (r2 == VT_LDOUBLE)
      func = TOK___fixxfdi;
    else if (r2 == VT_DOUBLE)
  ...
  if (func) {
    ...
    gfunc_call(1);
    vpushi(0);
    if (t == VT_LLONG) vtop->r2 = REG_LRET;
    vtop->r = REG_IRET;
    return;
  }
