...
void *tcc_get_symbol_err(TCCState *s, const char *name) {
  ...
  if (!sym) error("%s not defined", name);
  return sym;
}
...
#if 1
        printf(
            "new_bind=%x new_shndx=%x new_vis=%x old_bind=%x old_shndx=%x "
            "old_vis=%x\n",
...
static void put_stabs_r(const char *str, int type, int other, int desc,
  ...
  put_elf_reloc(symtab_section, stab_section,
                stab_section->data_offset - sizeof(unsigned int), R_DATA_32,
                sym_index);
}
  ...
  for (sym = (ElfW(Sym) *)symtab_section->data + 1; sym < sym_end; sym++) {
    ...
    if (sh_num == SHN_UNDEF) {
      ...
      if (do_resolve) {
        ...
        addr =
            (unsigned long)resolve_sym(s1, name, ELFW(ST_TYPE)(sym->st_info));
        if (addr) {
          sym->st_value = addr;
          goto found;
        }
  ...
  for (rel = qrel; rel < rel_end; rel++) {
    ...
    switch (type) {
      ...
      case R_X86_64_GOTPCREL:
        ...
        *(int *)ptr +=
            (s1->got->sh_addr - addr + s1->got_offsets[sym_index] - 4);
        break;
...
static void build_got(TCCState *s1) {
  ...
  s1->got->sh_entsize = 4;
  add_elf_sym(symtab_section, 0, 4, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
...
static void put_got_entry(TCCState *s1, int reloc_type, unsigned long size,
  ...
  if (s1->dynsym) {
    ...
    if (reloc_type == R_ARM_JUMP_SLOT) {
      ...
      /* if we build a DLL, we add a %ebx offset */
      if (s1->output_type == TCC_OUTPUT_DLL) error("DLLs unimplemented!");
...
static Section *new_symtab(TCCState *s1, const char *symtab_name, int sh_type,
...
static void add_init_array_defines(TCCState *s1, const char *section_name) {
  ...
  add_elf_sym(symtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
              s->sh_num, sym_start);
  add_elf_sym(symtab_section, end_offset, 0,
              ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, s->sh_num, sym_end);
}
  ...
  if (s1->do_bounds_check) {
    ...
    *ptr = 0;
    ...
    if (s1->output_type != TCC_OUTPUT_MEMORY) {
      ...
      put_elf_reloc(symtab_section, init_section, init_section->data_offset - 4,
                    R_386_PC32, sym_index);
    }
#endif
  }
#endif
...
static void tcc_add_linker_symbols(TCCState *s1) {
  ...
  add_elf_sym(symtab_section, text_section->data_offset, 0,
              ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, text_section->sh_num,
              "_etext");
  add_elf_sym(symtab_section, data_section->data_offset, 0,
              ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, data_section->sh_num,
              "_edata");
  add_elf_sym(symtab_section, bss_section->data_offset, 0,
              ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, bss_section->sh_num,
              "_end");
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    if (s->sh_type == SHT_PROGBITS && (s->sh_flags & SHF_ALLOC)) {
      ...
      for (;;) {
        ch = *p;
        if (!ch) break;
        if (!isid(ch) && !isnum(ch)) goto next_sec;
        p++;
      }
      snprintf(buf, sizeof(buf), "__start_%s", s->name);
      add_elf_sym(symtab_section, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE),
                  0, s->sh_num, buf);
      snprintf(buf, sizeof(buf), "__stop_%s", s->name);
      add_elf_sym(symtab_section, s->data_offset, 0,
...
int elf_output_file(TCCState *s1, const char *filename) {
  ...
  if (file_type != TCC_OUTPUT_OBJ) {
    ...
    if (!s1->static_link) {
      ...
      if (file_type == TCC_OUTPUT_EXE) {
        for (sym = (ElfW(Sym) *)symtab_section->data + 1; sym < sym_end;
             sym++) {
          if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (sym_index) {
              ...
              if (type == STT_FUNC) {
                put_got_entry(s1, R_JMP_SLOT, esym->st_size, esym->st_info,
                ...
                index =
                    put_elf_sym(s1->dynsym, offset, esym->st_size,
        ...
        for (sym = (ElfW(Sym) *)symtab_section->data + 1; sym < sym_end;
          ...
          if (ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
            ...
            if (ELFW(ST_TYPE)(sym->st_info) == STT_FUNC &&
              ...
              put_got_entry(s1, R_JMP_SLOT, sym->st_size, sym->st_info,
                            sym - (ElfW(Sym) *)symtab_section->data);
...
#endif
            {
              ...
              s1->symtab_to_dynsym[sym - (ElfW(Sym) *)symtab_section->data] =
                  index;
            }
          }
        }
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    } else if (s1->do_debug || file_type == TCC_OUTPUT_OBJ ||
    ...
    for (j = 0; j < 2; j++) {
      ...
      if (j == 0) {
        if (s1->output_format == TCC_OUTPUT_FORMAT_ELF) {
          ...
          file_offset =
              (file_offset + ELF_PAGE_SIZE - 1) & ~(ELF_PAGE_SIZE - 1);
        }
      }
    }
...
static int tcc_load_object_file(TCCState *s1, int fd,
    ...
    switch (s->sh_type) {
      case SHT_RELX:
        ...
        for (rel = (ElfW_Rel *)(s->data + offset); rel < rel_end; rel++) {
          ...
          invalid_reloc:
            error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x", i,
