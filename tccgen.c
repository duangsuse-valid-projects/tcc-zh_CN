...
static Sym *get_sym_ref(CType *type, Section *sec, unsigned long offset,
                        unsigned long size) {
  int v;
  Sym *sym;
...
int gv(int rc) {
  ...
  if (vtop->type.t & VT_BITFIELD) {
    ...
    if (is_float(vtop->type.t) &&
      ...
      if (size == LDOUBLE_SIZE) vtop->c.tab[2] &= 0xffff;
#endif
      ...
      check.d = 1;
      if (check.tab[0])
        for (i = 0; i < size; i++) ptr[i] = vtop->c.tab[size - 1 - i];
      else
#endif
    ...
    if ((vtop->type.t & VT_BTYPE) == VT_DOUBLE) vtop->r2 = r + 1;
#endif
  }
  return r;
}
...
void gen_opic(int op) {
  ...
  if (c1 && c2) {
    ...
    if (c2 && (((op == '*' || op == '/' || op == TOK_UDIV || op == TOK_PDIV) &&
                l2 == 1) ||
               ((op == '+' || op == '-' || op == '|' || op == '^' ||
...
void gen_cvt_itof1(int t) {
  if ((vtop->type.t & (VT_BTYPE | VT_UNSIGNED)) == (VT_LLONG | VT_UNSIGNED)) {
    if (t == VT_FLOAT) vpush_global_sym(&func_old_type, TOK___floatundisf);
#if LDOUBLE_SIZE != 8
...
void gen_cvt_ftoi1(int t) {
  ...
  if (t == (VT_LLONG | VT_UNSIGNED)) {
    ...
    if (st == VT_FLOAT) vpush_global_sym(&func_old_type, TOK___fixunssfdi);
#if LDOUBLE_SIZE != 8
      ...
      if (sf && df) {
        ...
        if (dbt == VT_BOOL) {
          ...
          if (dbt != (VT_INT | VT_UNSIGNED) &&
              dbt != (VT_LLONG | VT_UNSIGNED) && dbt != VT_LLONG)
            dbt = VT_INT;
          gen_cvt_ftoi1(dbt);
      ...
      switch (t) {
        ...
        case TOK_REGPARM2:
          ...
          if (n > 3)
            ...
            n = 0;
          if (n > 0) FUNC_CALL(ad->func_attr) = FUNC_FASTCALL1 + n - 1;
          skip(')');
          break;
        case TOK_FASTCALL1:
        case TOK_FASTCALL2:
        ...
        while (1) {
          ...
          if (bit_size >= 0) {
            ...
            if (bt != VT_INT && bt != VT_BYTE && bt != VT_SHORT &&
                bt != VT_BOOL && bt != VT_ENUM && bt != VT_LLONG)
            ...
            if (bit_size > bsize) {
              ...
              /* we do not have enough room ?
                 did the type change?
                 is it a union? */
              if ((bit_pos + bit_size) > bsize || bt != prevbt ||
                  a == TOK_UNION)
                bit_pos = 0;
              lbit_pos = bit_pos;
  ...
  while (1) {
    switch (tok) {
      ...
      case TOK_TYPEOF3:
        ...
        goto basic_type2;
      default:
        if (typespec_found || typedef_found) goto the_end;
        s = sym_find(tok);
    ...
    if (tok != ')') {
      for (;;) {
        ...
        if (l != FUNC_OLD) {
          ...
          if ((pt.t & VT_BTYPE) == VT_VOID) error("parameter declared as void");
  ...
  switch (tok) {
    ...
    tok_identifier:
      ...
      if ((s->type.t & (VT_STATIC | VT_INLINE | VT_BTYPE)) ==
        ...
        if (!s->c) put_extern_sym(s, text_section, 0, 0);
        r = VT_SYM | VT_CONST;
      } else {
        r = s->r;
      }
...
static void uneq(void) {
  ...
  if (tok == '=' || (tok >= TOK_A_MOD && tok <= TOK_A_DIV) ||
      tok == TOK_A_XOR || tok == TOK_A_OR || tok == TOK_A_SHL ||
      tok == TOK_A_SAR) {
    test_lvalue();
    t = tok;
    next();
...
static void expr_cmp(void) {
  ...
  expr_shift();
  while ((tok >= TOK_ULE && tok <= TOK_GT) || tok == TOK_ULT ||
         tok == TOK_UGE) {
    t = tok;
    next();
    expr_shift();
      ...
      switch (vtop->type.t & VT_BTYPE) {
        ...
        case VT_FUNC:
          for (p = vtop->type.ref; p; p = p->prev)
            if (p->prev == s) error("unsupported expression type");
      }
    }
...
static void decl_designator(CType *type, Section *sec, unsigned long c,
  ...
  elem_size = 0;
  nb_elems = 1;
  if (gnu_ext && (l = is_label()) != 0) goto struct_field;
  while (tok == '[' || tok == '.') {
    if (tok == '[') {
      ...
      next();
      index = expr_const();
      if (index < 0 || (s->c >= 0 && index >= s->c)) expect("invalid index");
      if (tok == TOK_DOTS && gnu_ext) {
        next();
        index_last = expr_const();
        if (index_last < 0 || (s->c >= 0 && index_last >= s->c) ||
            index_last < index)
...
static void init_putv(CType *type, Section *sec, unsigned long c, int v,
                      int expr_type) {
  ...
  if (type->t & VT_ARRAY) {
    ...
    if ((tok == TOK_LSTR &&
      ...
      while (tok == TOK_STR || tok == TOK_LSTR) {
        ...
        cstr_len--;
        nb = cstr_len;
        if (n >= 0 && nb > (n - array_length)) nb = n - array_length;
        if (!size_only) {
          if (cstr_len > nb)
...
static void decl_initializer_alloc(CType *type, AttributeDef *ad, int r,
  ...
  if ((r & VT_VALMASK) == VT_LOCAL) {
    sec = NULL;
    if (tcc_state->do_bounds_check && (type->t & VT_ARRAY)) loc--;
    loc = (loc - size) & -align;
    addr = loc;
    /* handles bounds */
    ...
    if (sec) {
      ...
      if (tcc_state->do_bounds_check) data_offset++;
...
void put_func_debug(Sym *sym) {
  ...
  snprintf(buf, sizeof(buf), "%s:%c1", funcname,
...
static void gen_function(Sym *sym) {
  ...
  func_ind = ind;
  /* put debug symbol */
  if (tcc_state->do_debug) put_func_debug(sym);
    ...
    while (1) { /* iterate thru each declaration */
      ...
      if (tok == '{') {
        ...
        if (sym) {
          ...
          if (FUNC_EXPORT(r)) FUNC_EXPORT(type.ref->r) = 1;
