...
static void gen_addr64(int r, Sym *sym, int64_t c) {
  if (r & VT_SYM) greloc(cur_text_section, sym, ind, R_X86_64_64);
  gen_le64(c);
}

/* output constant with relocation if 'r & VT_SYM' is true */
static void gen_addrpc32(int r, Sym *sym, int c) {
  if (r & VT_SYM) greloc(cur_text_section, sym, ind, R_X86_64_PC32);
  gen_le32(c - 4);
}
...
static void gen_modrm64(int opcode, int op_reg, int r, Sym *sym, int c) {
  ...
  if ((r & VT_VALMASK) != VT_CONST && (r & VT_VALMASK) != VT_LOCAL) {
...
static void gcall_or_jmp(int is_jmp) {
  ...
  if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
    ...
    if (vtop->r & VT_SYM) {
      ...
      put_elf_reloc(symtab_section, cur_text_section, ind + 1, R_X86_64_PC32,
                    0);
    }
...
static uint8_t arg_regs[6] = {TREG_RDI, TREG_RSI, TREG_RDX,
...
int gjmp(int t) { return psym(0xe9, t); }
...
int gtst(int inv, int t) {
  ...
  if (v == VT_CMP) {
    ...
    if ((v & 1) == inv) {
      ...
      p = &vtop->c.i;
      while (*p != 0) p = (int *)(cur_text_section->data + *p);
      *p = t;
      t = vtop->c.i;
    } else {
      t = gjmp(t);
  ...
  switch (op) {
    ...
    gen_op8:
      if ((vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST &&
        ...
        r = vtop[-1].r;
        fr = vtop[0].r;
        if (opc != 7 || is64_type(vtop[0].type.t) ||
            (vtop[0].type.t & VT_UNSIGNED) || is64_type(vtop[-1].type.t) ||
...
void gen_opf(int op) {
  ...
  if ((vtop[0].r & (VT_VALMASK | VT_LVAL)) == VT_CONST) gv(float_type);
  ...
  if ((vtop->type.t & VT_BTYPE) == VT_LDOUBLE) {
    ...
    if (op >= TOK_ULT && op <= TOK_GT) {
      ...
      if (op == TOK_EQ || op == TOK_NE) {
        ...
        if (op == TOK_LE || op == TOK_LT) swapped = !swapped;
