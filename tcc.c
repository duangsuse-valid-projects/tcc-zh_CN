...
void help(void) {
  printf(
      ...
      "usage: tcc [-v] [-c] [-o outfile] [-Bdir] [-bench] [-Idir] "
      "[-Dsym[=val]] [-Usym]\n"
      "           [-Wwarn] [-g] [-b] [-bt N] [-Ldir] [-llib] [-shared] "
      "[-soname name]\n"
      ...
      "  -Wwarning   set or reset (with 'no-' prefix) 'warning' (see man "
      "page)\n"
  ...
  while (optind < argc) {
    ...
    option_found:
      ...
      switch (popt->index) {
        ...
        case TCC_OPTION_v:
          do {
            if (0 == s->verbose++) printf("tcc version %s\n", TCC_VERSION);
          } while (*optarg++ == 'v');
          break;
        case TCC_OPTION_f:
...
int main(int argc, char **argv) {
  ...
  /* if outfile provided without other options, we output an
     executable */
  if (outfile && output_type == TCC_OUTPUT_MEMORY) output_type = TCC_OUTPUT_EXE;
  ...
  if (do_bench) tcc_print_stats(s, getclock_us() - start_time);
...
the_end:
  ...
  if (!s->do_bounds_check) tcc_delete(s);
