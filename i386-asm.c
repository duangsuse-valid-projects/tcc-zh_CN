...
#define DEF_ASM_OP0L(name, opcode, group, instr_type) \
  {TOK_ASM_##name, opcode, (instr_type | group << OPC_GROUP_SHIFT), 0},
#define DEF_ASM_OP1(name, opcode, group, instr_type, op0) \
  {TOK_ASM_##name, opcode, (instr_type | group << OPC_GROUP_SHIFT), 1, {op0}},
#define DEF_ASM_OP2(name, opcode, group, instr_type, op0, op1) \
  {TOK_ASM_##name,                                             \
   opcode,                                                     \
   (instr_type | group << OPC_GROUP_SHIFT),                    \
   2,                                                          \
   {op0, op1}},
...
static void parse_operand(TCCState *s1, Operand *op) {
  ...
  if (tok == '%') {
    ...
    if (tok >= TOK_ASM_al && tok <= TOK_ASM_db7) {
      ...
      if (tok == '(') {
        next();
        if (tok != TOK_PPNUM) goto reg_error;
        p = tokc.cstr->data;
        reg = p[0] - '0';
        if ((unsigned)reg >= 8 || p[1] != '\0') goto reg_error;
        op->reg = reg;
        next();
        skip(')');
      }
      if (op->reg == 0) op->type |= OP_ST0;
      goto no_skip;
    } else {
    reg_error:
  ...
  no_skip:;
    ...
    op->e.sym = e.sym;
    if (!op->e.sym) {
      if (op->e.v == (uint8_t)op->e.v) op->type |= OP_IM8;
      if (op->e.v == (int8_t)op->e.v) op->type |= OP_IM8S;
      if (op->e.v == (uint16_t)op->e.v) op->type |= OP_IM16;
    }
  } else {
    ...
    if (op->reg == -1 && op->reg2 == -1) op->type |= OP_ADDR;
  }
  op->type |= indir;
}
...
static inline void asm_modrm(int reg, Operand *op) {
  ...
  if (op->type & (OP_REG | OP_MMX | OP_SSE)) {
    ...
    if (reg1 == 4) {
      /* add sib byte */
      reg2 = op->reg2;
      if (reg2 == -1) reg2 = 4; /* indicate no index */
...
static void asm_opcode(TCCState *s1, int opcode) {
  ...
  for (pa = asm_instrs; pa->sym != 0; pa++) {
    ...
    if (pa->instr_type & OPC_FARITH) {
      v = opcode - pa->sym;
      if (!((unsigned)v < 8 * 6 && (v % 6) == 0)) continue;
    } else if (pa->instr_type & OPC_ARITH) {
      if (!(opcode >= pa->sym && opcode < pa->sym + 8 * 4)) continue;
      goto compute_size;
    } else if (pa->instr_type & OPC_SHIFT) {
      if (!(opcode >= pa->sym && opcode < pa->sym + 7 * 4)) continue;
      goto compute_size;
    } else if (pa->instr_type & OPC_TEST) {
      if (!(opcode >= pa->sym && opcode < pa->sym + NB_TEST_OPCODES)) continue;
    } else if (pa->instr_type & OPC_B) {
      if (!(opcode >= pa->sym && opcode <= pa->sym + 3)) continue;
    compute_size:
      s = (opcode - pa->sym) & 3;
    } else if (pa->instr_type & OPC_WL) {
      if (!(opcode >= pa->sym && opcode <= pa->sym + 2)) continue;
      s = opcode - pa->sym + 1;
    } else {
      if (pa->sym != opcode) continue;
    }
    if (pa->nb_ops != nb_ops) continue;
    ...
    for (i = 0; i < nb_ops; i++) {
      ...
      if (op1 & OPT_EA) v |= OP_EA;
      op_type[i] = v;
      if ((ops[i].type & v) == 0) goto next;
    }
    /* all is matching ! */
    break;
  next:;
  ...
  if (pa->instr_type & OPC_FWAIT) g(0x9b);
  if (seg_prefix) g(seg_prefix);
  ...
  if (pa->instr_type & OPC_MODRM) {
    ...
    for (i = 0; i < nb_ops; i++) {
      if (op_type[i] & OP_EA) goto modrm_found;
    }
    /* then if not found, a register or indirection (shift instructions) */
    for (i = 0; i < nb_ops; i++) {
      if (op_type[i] & (OP_REG | OP_MMX | OP_SSE | OP_INDIR)) goto modrm_found;
    }
#ifdef ASM_DEBUG
    error("bad op table");
#endif
  ...
  if (pa->opcode == 0x9a || pa->opcode == 0xea) {
    ...
    for (i = 0; i < nb_ops; i++) {
      ...
      if (v & (OP_IM8 | OP_IM16 | OP_IM32 | OP_IM8S | OP_ADDR)) {
        ...
        if (v & (OP_IM8 | OP_IM8S)) {
          if (ops[i].e.sym) goto error_relocate;
          g(ops[i].e.v);
        } else if (v & OP_IM16) {
          if (ops[i].e.sym) {
          error_relocate:
...
static void asm_compute_constraints(ASMOperand *operands, int nb_operands,
  ...
  try_next:
    ...
    switch (c) {
      ...
      case 'D':
        reg = 7;
      alloc_reg:
        if (is_reg_allocated(reg)) goto try_next;
        goto reg_found;
      case 'q':
        /* eax, ebx, ecx or edx */
        for (reg = 0; reg < 4; reg++) {
      ...
      case 'g':
        ...
        break;
      default:
        error("asm constraint %d ('%s') could not be satisfied", j,
              op->constraint);
        break;
    }
  ...
  *pout_reg = -1;
  ...
  if (*pout_reg >= 0) printf("out_reg=%d\n", *pout_reg);
#endif
}
...
static void subst_asm_operand(CString *add_str, SValue *sv, int modifier) {
  ...
  if ((r & VT_VALMASK) == VT_CONST) {
    ...
    reg = r & VT_VALMASK;
    if (reg >= VT_CONST) error("internal compiler error");
    ...
    if ((sv->type.t & VT_BTYPE) == VT_BYTE)
      ...
      size = 4;
    if (size == 1 && reg >= 4) size = 4;

    if (modifier == 'b') {
      if (reg >= 4) error("cannot use byte register");
      size = 1;
    } else if (modifier == 'h') {
      if (reg >= 4) error("cannot use byte register");
      size = -1;
...
static void asm_gen_code(ASMOperand *operands, int nb_operands, int nb_outputs,
  ...
  for (i = 0; i < nb_operands; i++) {
    op = &operands[i];
    if (op->reg >= 0) regs_allocated[op->reg] = 1;
  }
  if (!is_output) {
    /* generate reg save code */
    for (i = 0; i < NB_SAVED_REGS; i++) {
      reg = reg_saved[i];
      if (regs_allocated[reg]) g(0x50 + reg);
    }
    ...
    for (i = NB_SAVED_REGS - 1; i >= 0; i--) {
      reg = reg_saved[i];
      if (regs_allocated[reg]) g(0x58 + reg);
    }
  }
}
...
static void asm_clobber(uint8_t *clobber_regs, const char *str) {
  ...
  if (!strcmp(str, "memory") || !strcmp(str, "cc")) return;
