...
static void asm_expr(TCCState *s1, ExprValue *pe) { asm_expr_sum(s1, pe); }
...
static void asm_new_label1(TCCState *s1, int label, int is_local, int sh_num,
                           int value) {
  Sym *sym;
...
static void asm_parse_directive(TCCState *s1) {
  ...
  switch (tok) {
    ...
    case TOK_ASM_org: {
      ...
      n = asm_int_expr(s1);
      if (n < ind) error("attempt to .org backwards");
      v = 0;
      size = n - ind;
      goto zero_pad;
    } break;
...
static void subst_asm_operands(ASMOperand *operands, int nb_operands,
                               int nb_outputs, CString *out_str,
                               CString *in_str) {
